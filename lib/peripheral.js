...
function dataReadComplete( data, centralId ) {
    ...
    // Ignore pairing broadcasts.
    if ( data.includes( settings.data.package.seperator ) ) {
        return;
    }
    ...
    datastore.getPackagesForDeviceAndRecipient( centralId, data, function ( result, error ) {
        ...
        if ( result.rowCount > 0 ) {
            ...
            result.rows.forEach( function ( item ) {
                ...
                datastore.updatePackageSendCount( item.id, 1 );
            } );

            var pack = pack_list.join( settings.data.package.seperator );
            dataSendToCentral( pack, centralId );
    ...
    handle: function handle( error ) {
        ...
        if ( !error ) {
            ...
            var characteristic = new Characteristic( {
                uuid: settings.uuids.characteristics.pure,
            ...
            var primaryService = new PrimaryService( {
                uuid: settings.uuids.service.pure,
