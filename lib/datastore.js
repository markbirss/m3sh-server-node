...
var self = module.exports = {
    ...
    getPackagesForDeviceAndRecipient: function getPackagesForDeviceAndRecipient( deviceId, recipient, callback ) {
        ...
        let sql = "SELECT * FROM packages LEFT OUTER JOIN transfers ON (transfers.package = packages.id) WHERE transfers.device != $1 AND ((count < $2) AND ((transfers.package IS NULL) OR (recipient = $2)))";
    ...
    insertPackage: function insertPackage( pack, callback ) {
        ...
        if ( pack.length > 64 ) {
            ...
            let sql = "INSERT INTO packages(id, sender, recipient, body, count, expiration, tags) VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT DO NOTHING returning id";
    ...
    addTransferRecord: function addTransferRecord( packId, deviceId, callback ) {
        ...
        let sql = "INSERT INTO transfers (package, device, expiration) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING returning package";
